    ...
    try:
        ...
        if response['status'] == '1':
            ...
            for tx in transactions:
                ...
                if value > 500_000:
                    to_address = tx['to']
                    tx_type = "🔥 **به صرافی (فروش احتمالی)**" if "Binance" in to_address or "KuCoin" in to_address else "❄️ **به کیف پول (نگهداری)**"
...
def generate_full_report_data(symbol):
    screener = "crypto"
    if symbol.upper() in ["XAUUSD", "EURUSD"]: screener = "forex"
    try:
        ...
        logging.error(f"Failed to get TV data for {symbol}: {e}")
        return None
    ...
    if query_data.startswith('main_menu'):
        ...
        if section == 'structure':
            ...
            report_section += f"**تحلیل TradingView (1H):** `{data['summary_1h']['RECOMMENDATION']}`\n"
             ...
             report_section += investpy.economic_calendar(countries=['united states']).to_string()
